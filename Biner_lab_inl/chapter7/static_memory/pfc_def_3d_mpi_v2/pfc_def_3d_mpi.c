...
int main(int argc, char **argv){
	...
	for(int istep=0;istep<=nstep;istep++){
		...
		if(fmod(istep,ndefor)==0){
			...
			prepare_fft_3d_mpi(Nx,Ny,Nz,dx,dy,dz,kx,ky,kz,k2,k4,local_n0,local_0_start); //get FFT coefficients
		}
		...
		if(fmod(istep,nprint)==0){
			...
			for(int i=0;i<local_n0;i++){
				for(int j=0;j<Ny;j++){
					for(int k=0;k<Nz;k++){
						...
						ff_out[local_0_start+i][j][k]=ff[ii][0];
					}
				}
			}
			...
			MPI_Gather(den_out[local_0_start], local_n0*Ny*Nz, MPI_DOUBLE, den_out[local_0_start], local_n0*Ny*Nz, MPI_DOUBLE, 0, MPI_COMM_WORLD);
			MPI_Gather(ff_out[local_0_start], local_n0*Ny*Nz, MPI_DOUBLE, ff_out[local_0_start], local_n0*Ny*Nz, MPI_DOUBLE, 0, MPI_COMM_WORLD);
			//
			...
			if (rank == 0){
				...
				write_vtk_grid_values_3D(Nx,Ny,Nz,dx,dy,dz,istep,den_out,ff_out);
