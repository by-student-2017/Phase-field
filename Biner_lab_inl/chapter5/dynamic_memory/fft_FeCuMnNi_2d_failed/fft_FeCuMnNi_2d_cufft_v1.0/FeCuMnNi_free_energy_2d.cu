#include <math.h> //M_PI
#include <stdlib.h> //rand()

/* Note
  Although we are using an array of complex numbers,
   the imaginary component of the input and output in this function is 0.
  
  This function needs to be rewritten depending on the system.
  
  If the systems are different, you will rewrite the free energy and
   its derivative obtained from CALPHAD.
  
  I have included comments so that if you understand it while using it in 
   conjunction with the textbook, it will not be difficult to rewrite it.
  
  Good luck !
*/

/* Variable and array list
  Nx: Number of grid points in the x-direction
  Ny: Number of grid points in the y-direction
  tempr: Temperature [K]
  cu(Nx,Ny): Modulated Cu values at the grid points
  mn(Nx,Ny): Modulated Mn values at the grid points
  ni(Nx,Ny): Modulated Ni values at the grid points
  orp(Nx,Ny): Modulated order parameter values at the grid points
  dgdcu(Nx,Ny): Functional derivative with respect to Cu
  dgdmn(Nx,Ny): Functional derivative with respect to Mn
  dgdni(Nx,Ny): Functional derivative with respect to Ni
  dgdor(Nx,Ny): Functional derivative with respect to
    non-conserved order parameter */

void FeCuMnNi_free_energy_2d(int Nx, int Ny, 
	float *cu, float *mn, float *ni, float *orp, float tempr,
	float *dgdcu, float *dgdmn, float *dgdni, float *dgdor){
	
	float R=8.314472; //The value of gas constant, R [J/mol/K]
	float T=tempr; //Temperature [K]
	float RT=R*T; //RT [J/mol], 6842.810456 [J/mol] for 823.0 [K]
	float beta=1.0/RT; //inverse temperature, 1.4613878411949395e-4 for 823.0 [K]
	
	float constw=5.0e3/RT; //The value W in Eq.5.25(Total free energy, F). The W is [J/mol] unit
		/* The term W*g(eta) in the integrand represents the energy barrier for
		   the phase transformation between the alpha and gamma phases. g(eta)=eta*(1-eta) */
	float Vm=7.09e-6; //Molar volume [m^3/mol]
	float Es=214.0e9; //Energy function of elastic stifness, nGPa
	float conste=Es*Vm/RT; //The normalized value of Y in Eq.5.25(Total free energy, F)
	
	//Lattice mismatch (=lattice misfit)
	//Misfit strain values in Eq.5.26 for each alloying element
	float eta2=3.29e-2;
	float eta3=5.22e-4;
	float eta4=4.75e-4;
	
	//Initial concentrations of alloying elements
	float c02=0.15; //average Cu concentration
	float c03=0.01; //average Mn concentration
	float c04=0.01; //average Ni concentration
	
	//Value of coefficient which will be used as a penalty parameter later
	float Coef=10.0;
	
	//----- ----- ----- ----- ----- ----- ----- -----
	// alpha phase
	//Gibbs energy (Memo: ln(T)=log(T) on C language)
	float G0A1 = 0.0;
	float G0A2 =   4017.0 + (-1.255*T); 
		//2984.135 for 823.0 [K]
	float G0A3 =  -3235.3 +  127.85*T + (-23.7*T*log(T)) + (-0.00742717*T*T) + 60000.0/T;
		//-33909.365609 for 823.0 [K]
	float G0A4 = 8175.084 + (-3.556*T);
		//5248.496 for 823.0 [K]
	//
	float LA12 =  41033.0 + (-6.022*T); 
		//36076.894 for 823.0 [K]
	float LA13 =  -2759.0 +   1.237*T;
		//-1740.949 for 823.0 [K]
	float LA14p=(1789.03-1.92912*T); 
	float LA14;
	//float LA14 = -956.63-1.28726*T + LA14p*(c1-c4);
		//-2016.04498 + 201.36424*(c1-c4) for 823.0 [K]
	float LA23p=-9865.0;
	float LA23;
	//float LA23 =     11190.0-6.0*T + LA23p*(c2-c3);
		//6252.0 - 9865.0*(c2-c3) for 823.0 [K]
	float LA24 =     8366.0+2.80*T;
		//10670.4 for 823.0 [K]
	float LA34p=6276.0;
	float LA34;
	//float LA34 =  -51638.31+3.64*T + LA34p*(c3-c4);
		//-48642.59 + 6276.0*(c3-c4) for 823.0 [K]
	//
	float LA123=30000.0;
	//float LA124=0.0;
	//float LA134=0.0;
	//float LA234=0.0;
	//----- ----- ----- ----- ----- ----- ----- -----
	// gamma phase
	//Gibbs energy (Memo: ln(T)=log(T) on C language)
	float G0G1 =      -1462.4+8.282*T +    (-1.15*T*log(T)) +  6.4e-4*T*T;
		//3027.89638596 for 823.0 [K]
	float G0G2 = 0.0;
	float G0G3 =    -3439.3+131.884*T + (-24.5177*T*log(T)) + (-0.006*T*T) + 69600.0/T;
		//42294.693153 got 823.0 [K]
	float G0G4 = 0.0;
	//
	float LG12p=(11512.0-7.095*T);
	float LG12;
	//float LG12 =     53360.0-12.626*T + LG12p*(c2-c1);
		//42968.802 + 5672.815*(c2-c1) for 823.0 [K]
	float LG13p=-259.0;
	float LG13;
	//float LG13 =        -7762+3.865*T + LG13p*(c1-c3);
		//-4581.105 - 259.0*(c1-c3) for 823.0 [K]
	float LG14p1=(11082.1315-4.45077*T);
	float LG14p2=-725.8051;
	float LG14;
	//float LG14 = -12054.335+3.27413*T + LG14p1*(c1-c4) + LG14p2*(c1-c4)*(c1-c4);
		//-9359.72601 + 7419.14779*(c1-c4) -725.8051*(c1-c4)*(c1-c4) for 823.0 [K]
	float LG23p1=(-10600.0+3.0*T);
	float LG23p2=(-4850.0+3.5*T);
	float LG23;
	//float LG23 =        11820.0-2.3*T + LG23p1*(c2-c3) + LG23p2*(c2-c3)*(c2-c3)*(c2-c3);
		//9927.1 + -8131.0*(c2-c3) + -1969.5*(c2-c3)*(c2-c3)*(c2-c3) for 823.0 [K]
	float LG24p=(-4359.6+1.812*T);
	float LG24;
	//float LG24 =       8366.0+2.802*T + LG24p*(c2-c4);
		//10672.046 + -2868.324*(c2-c4) for 823.0 [K]
	float LG34p=6276.0;
	float LG34;
	//float LG34 =    -58158.0+10.878*T + LG34p*(c3-c4);
		//-49205.406 + 6276*(c3-c4) for 823.0 [K]
	//
	float LG123 =     -68000.0+50.0*T;
		//-26850.0 for 823.0 [K]
	float LG124 =     -73272.0+30.9*T;
		//-47841.3 for 823.0 [K]
	//----- ----- ----- ----- ----- ----- ----- -----	
	int ii;
	
	float c1,c2,c3,c4;
	float funch; //h(eta)
	float funcg; //g(eta)
	float cc0eta; //sum(etai*(ci-c0i))
	float elaste;
	float delascu, delasmn, delasni;
	float dgcua, dgmna, dgnia; // dG/dc for alpha phase
	float dgcug, dgmng, dgnig; // dG/dc for gamma phase
	float gcuni;
	float gcunia; // sum(Ga(ci)) of alpha phase
	float gcunig; // sum(Gg(ci)) of gamma phase
	float thrc=0.0;
	float dgcu;
	float dhdor;
	
	for(int i=0;i<Nx;i++){
		for(int j=0;j<Ny;j++){
			ii=i*Ny+j;
			
			//Current values of the alloying elements at the curent grid point
			c2=cu[ii]; // concentration of Cu(x,y,z)
			c3=mn[ii]; // concentration of Mn(x,y,z)
			c4=ni[ii]; // concentration of Ni(x,y,z)
			c1=1.0-c2-c3-c4; // concentration of Fe(x,y,z)
			
			//Evaluate the value of function h(eta)=eta*eta*(3-2*eta), eta=orp[ii][0]
			funch=orp[ii]*orp[ii]*(3.0-2.0*orp[ii]);
			
			//Evaluate the value of function g(eta)=eta*(1-eta), eta=orp[ii][0]
			funcg=orp[ii]*(1.0-orp[ii]);
			
			//This is related with Eq.5.26
			cc0eta=( eta2*(c2-c02) + eta3*(c3-c03) + eta4*(c4-c04) );
			
			//Calculate the elastic energy
			elaste=conste*cc0eta*cc0eta; //Y*Vm*epsilon0(ci)^2 in Eq.5.25
			
			//Derivative of elastic energy with respect to each alloying element
			/* The quantitiy Y*Vm*epsilon0(ci)^2 is the elastic strain energy
			   include from the coherent phase separation, 
			   where Y is the average stifness and Vm is the molar volume and
			   epsilon0(ci) is the eigenstrain and taken as Eq.5.26 */
			delascu=conste*2.0*cc0eta*eta2; // d(elaste)/d(c2)
			delasmn=conste*2.0*cc0eta*eta3; // d(elaste)/d(c3)
			delasni=conste*2.0*cc0eta*eta4; // d(elaste)/d(c4)
			
			//parameters
			//----- ----- ----- -----
			// alpha phase
			//Gibbs energy (Memo: ln(T)=log(T) on C language)
			G0A1 =      0.0;
			G0A2 =   4017.0 + (-1.255*T); 
			G0A3 =  -3235.3 +  127.85*T + (-23.7*T*log(T)) + (-0.00742717*T*T) + 60000.0/T;
			G0A4 = 8175.084 + (-3.556*T);
			//
			LA12 =  41033.0 + (-6.022*T); 
			LA13 =  -2759.0 +   1.237*T;
			LA14p=(1789.03-1.92912*T); 
			LA14 =    -956.63-1.28726*T + LA14p*(c1-c4);
			LA23p=-9865.0;
			LA23 =        11190.0-6.0*T + LA23p*(c2-c3);
			LA24 =        8366.0+2.80*T;
			LA34p=6276.0;
			LA34 =     -51638.31+3.64*T + LA34p*(c3-c4);
			//
			LA123=  30000.0;
			//LA124=      0.0;
			//LA134=      0.0;
			//LA234=      0.0;
			//----- ----- ----- -----
			// gamma phase
			//Gibbs energy (Memo: ln(T)=log(T) on C language)
			G0G1 =      -1462.4+8.282*T +    (-1.15*T*log(T)) +  6.4e-4*T*T;
			G0G2 =      0.0;
			G0G3 =    -3439.3+131.884*T + (-24.5177*T*log(T)) + (-0.006*T*T) + 69600.0/T;
			G0G4 =      0.0;
			//
			LG12p=(11512.0-7.095*T);
			LG12 =     53360.0-12.626*T + LG12p*(c2-c1);
			LG13p=-259.0;
			LG13 =        -7762+3.865*T + LG13p*(c1-c3);
			LG14p1=(11082.1315-4.45077*T);
			LG14p2=-725.8051;
			LG14 = -12054.335+3.27413*T + LG14p1*(c1-c4) + LG14p2*(c1-c4)*(c1-c4);
			LG23p1=(-10600.0+3.0*T);
			LG23p2=(-4850.0+3.5*T);
			LG23 =        11820.0-2.3*T + LG23p1*(c2-c3) + LG23p2*(c2-c3)*(c2-c3)*(c2-c3);
			LG24 =       8366.0+2.802*T + LG24p*(c2-c4);
			LG34p=6276.0;
			LG34 =    -58158.0+10.878*T + LG34p*(c3-c4);
			//
			LG123 =     -68000.0+50.0*T;
			LG124 =     -73272.0+30.9*T;
			//----- ----- ----- -----
			
			//If Fe concentration is within the limits continue
			if(c1<0.9995 && c1>0.0005){
				//
				// 1:Fe, 2:Cu, 3:Mn, 4:Ni
				
				// Calculate the functional derivatives for alpha phase
				//
				// alpha phase
				//----- ----- ----- ----- ----- -----
				//Functional derivative with respect to Cu concentration
				//dGa/dCcu=dGa/dc2 (original in text)
				//dgcua=1.4613878411949395e-4*
				//	(-201.364240*c1*c4
				//	-(201.364240*(-2*c4-c3-c2+1.0)-2016.04498)*c4
				//	+10672.046*c4
				//	+30000.0*c3*c1
				//	+36076.894*c1
				//	+6842.810456*(log(c2)-log(c1))
				//	+(6252.0-9865.0*(c2-c3))*c3
				//	-39865.0*c2*c3
				//	+1740.949*c3
				//	-36076.894*c2
				//	+2984.135);
				//----- -----
				//dGa/dCcu=dGa/dc2 (modified original code)
				//d(LA14)*c1*c4=-LA14p*c1*c4
				//d(LA23)*c2*c3=LA23p*c2*c3
				//=G0A2-G0A1
				//+(-LA14p*c1*c4-LA12*c2-LA13*c3-LA14*c4 +LA12*c1 +LA23p*c2*c3+LA23*c3 +LA24*c4)
				//-LA123*c2*c3+LA123*c1*c3
				//+Gmg(omitted)+-RT*log(c1)-RT+RT*log(c2)+RT
				//Gmg:Ref https://doi.org/10.2320/matertrans.47.2765
				dgcua=beta*
					(G0A2-G0A1
					-LA14p*c1*c4
					-LA12*c2 -LA13*c3 -LA14*c4
					+LA12*c1
					+LA23p*c2*c3 ///a part of -39865.0*c2*c3
					+LA23*c3
					+LA24*c4
					-LA123*c2*c3 ///a part of -39865.0*c2*c3
					+LA123*c1*c3
					+RT*(log(c2)-log(c1))
					);
				//----- ----- ----- ----- ----- -----
				//Functional derivative with respect to Mn concentration
				//dGa/dCmn=dGa/dc3 (original in text)
				//dgmna=1.4613878411949395e-4*
				//	(-201.364240*c1*c4
				//	+(6276.0*(c3-c4)-48642.59)*c4
				//	-(201.364240*(-2*c4-c3-c2+1.0)-2016.04498)*c4
				//	+6276.0*c3*c4
				//	+30000.0*c2*c1
				//	-1740.949*c1
				//	+6842.810456*(log(c3)-log(c1))
				//	+20135.0*c2*c3
				//	+1740.949*c3
				//	+c2*(6252.0-9865.0*(c2-c3))
				//	-36076.894*c2
				//	-33929.89130169578);
				//----- -----
				//dGa/dCmn=dGa/dc3 (modified original code)
				//d(LA14)*c1*c4=-LA14p*c1*c4
				//d(LA23)*c2*c3=-LA23p*c2*c3
				//d(LA34)*c3*c4=LA34p*c3*c4
				//=G0A3-G0A1
				//+(-LA14p*c1*c4-LA12*c2-LA13*c3-LA14*c4 +LA13*c1 -LA23p*c2*c3+LA23*c2 +LA34p*c3*c4+LA34*c4)
				//-LA123*c2*c3+LA123*c1*c2
				//+Gmg(omitted)+-RT*log(c1)-RT+RT*log(c3)+RT
					// LA123*c1*c2*c3=LA123*c1*c2-LA123*c2*c3
				dgmna=beta*
					(G0A3-G0A1
					-LA14p*c1*c4
					-LA12*c2 -LA13*c3 -LA14*c4
					+LA13*c1
					-LA23p*c2*c3 //a part of 20135.0*c2*c3 (but not positive value) = (-LA23p-LA123)*c2*c3
					+LA23*c2
					+LA34p*c3*c4 +LA34*c4
					-LA123*c2*c3 ///a part of 20135.0*c2*c3 (but not positive value) = (-LA23p-LA123)*c2*c3
					+LA123*c1*c2
					+RT*(log(c3)-log(c1))
					);
				//----- ----- ----- ----- ----- -----
				//Functional derivative with respect to Ni concentration
				//dGa/dCni=dGa/dc4 (original in text)
				//dgnia=1.4613878411949395e-4*
				//	(6842.810456*(log(c4)-log(c1))
				//	-402.7284800*c1*c4
				//	-(201.364240*(-2*c4-c3-c2+1.0)-2016.04498)*c4
				//	-6276.0*c3*c4
				//	+(201.3642400*(-2*c4-c3-c2+1.0)-2016.04498)*c1
				//	+c3*(6276.0*(c3-c4)-48642.59)
				//	-30000.0*c2*c3
				//	+1740.949*c3
				//	-25404.848*c2
				//	+5788.49600);
				//----- -----
				//dGa/dCni=dGa/dc4 (modified original code)
				//d(LA14)*c1*c4=LA14p*-2*c1*c4
				//d(LA34)*c3*c4=-LA34p*c3*c4
				//=G0A4-G0A1
				//+(LA14p*-2*c1*c4-LA12*c2-LA13*c3-LA14*c4 +LA14*c1 +LA24*c2 -LA34p*c3*c4+LA34*c3)
				//-LA123*c2*c3
				//+Gmg(omitted)+-RT*log(c1)-RT+RT*log(c4)+RT
				dgnia=beta*
					(G0A4-G0A1 //+5788.49600 ?
					+LA14p*-2.0*c1*c4
					-LA12*c2 //a part of -25404.848*c2 = (-LA12+LA24)*c2
					-LA13*c3
					-LA14*c4
					+LA14*c1
					+LA24*c2  //a part of -25404.848*c2 = (-LA12+LA24)*c2
					-LA34p*c3*c4 +LA34*c3
					-LA123*c2*c3
					+RT*(log(c4)-log(c1))
					);
				//----- ----- ----- ----- ----- -----
				
				// Calculate the functional derivatives for gamma phase
				//
				// gamma phase
				//----- ----- ----- ----- ----- -----
				//Functional derivative with respect to Cu concentration
				//dGg/dCcu=dGg/dc2 (original in text)
				//dgcug=1.4613878411949395e-4*
				//	(c1*(5672.81500*(c4+c3+2*c2-1.0)+42968.802)
				//	-c2*(5672.81500*(c4+c3+2*c2-1.0)+42968.802)
				//	+(1451.610348*(-2*c4-c3-c2+1.0)-7419.147789)*c1*c4
				//	-47841.3*c1*c4
				//	+(10672.046-2868.3240*(c2-c4))*c4
				//	-(-725.805174*(-2*c4-c3-c2+1.0)*(-2*c4-c3-c2+1.0)+7419.14778*(-2*c4-c3-c2+1.0)-9359.746009)*c4
				//	+44972.976*c2*c4
				//	-26591.0*c3*c1
				//	+11345.63*c2*c1
				//	-c3*(-259.0*(-c4-2*c3-c2+1.0)-4581.105)
				//	+6842.810456*(log(c2)-log(c1))
				//	+(-1969.5*(c2-c3)*(c2-c3)*(c2-c3) -8131.0*(c2-c3) +9927.0)*c3
				//	+c2*(-5908.5*(c2-c3)*(c2-c3)-8131.0)*c3
				//	+26850.0*c2*c3
				//	+566.3008361308123);
				//----- -----
				//dGg/dCcu=dGg/dc2 (modified original code)
				//d(LG12)*c1*c2=LG12p*2.0*c1*c2
				//d(LG13)*c1*c3=-LG13p*c1*c3
				//d(LG14)*c1*c4=-LG14p1*c1*c4-LG14p2*2.0*(c1-c4)*c1*c4
				//d(LG23)*c2*c3=LG23p1*c2*c3+LG23p2*3.0*(c2-c3)*(c2-c3)*c2*c3
				//d(LG24)*c2*c4=LG24p*c2*c4
				//=G0G2-G0G1
				//+(LG12p*2.0*c1*c2-LG13p*c1*c3-LG14p1*c1*c4-LG14p2*2.0*(c1-c4)*c1*c4
				//-LG12*c2-LG13*c3-LG14*c4 +LG12*c1 +LG23*c3 +LG24*c4
				//+LG23p1*c2*c3+LG23p2*3.0*(c2-c3)*(c2-c3)*c2*c3+LG24p*c2*c4)
				//-LG123*c2*c3+LG123*c1*c3 -LG124*c2*c4+LG124*c1*c4
				//+Gmg(omitted)+-RT*log(c1)-RT+RT*log(c2)+RT
				//Gmg:Ref https://doi.org/10.2320/matertrans.47.2765
				dgcug=beta*
					(G0G2-G0G1 //+566.3008361308123 ?
					+LG12p*2.0*c1*c2
					-LG13p*c1*c3 //a part of -26591.0*c1*c3 = (-LG13p+LG123)*c1*c3
					-LG14p1*c1*c4-LG14p2*2.0*(c1-c4)*c1*c4
					-LG12*c2
					-LG13*c3
					-LG14*c4
					+LG12*c1
					+LG23p1*c2*c3+LG23p2*3.0*(c2-c3)*(c2-c3)*c2*c3
					+LG23*c3
					+LG24p*c2*c4 //a part of +44972.976*c2*c4 = (LG24p-LG124)*c2*c4
					+LG24*c4
					-LG124*c2*c4 //a part of +44972.976*c2*c4 = (LG24p-LG124)*c2*c4
					+LG124*c1*c4 
					-LG123*c2*c3
					+LG123*c1*c3 //a part of -26591.0*c1*c3 = (-LG13p+LG123)*c1*c3
					+RT*(log(c2)-log(c1))
					);
				//----- ----- ----- ----- ----- -----
				//Functional derivative with respect to Mn concentration
				//dGg/dCmn=dGg/dc3 (original in text)
				//dgmng=1.4613878411949395e-4*(
				//	-c2*(5672.8150*(c4+c3+2*c2-1.0)+42968.802)
				//	+(1451.610348*(-2*c4-c3-c2+1.0)-7419.14778999)*c1*c4
				//	+(6276.0*(c3-c4)-49205.406)*c4
				//	-(-725.805174*(-2*c4-c3-c2+1.0)*(-2*c4-c3-c2+1.0)+7419.14778999*(-2*c4-c3-c2+1.0)-9359.7460999)*c4
				//	+6276.0*c3*c4
				//	+47841.3*c2*c4
				//	+(-259.0*(-c4-2*c3-c2+1.0)-4581.105)*c1
				//	+518.0*c3*c1
				//	-21177.185*c2*c1
				//	-c3*(-259.0*(-c4-2*c3-c2+1.0)-4581.105)
				//	+6842.81045*(log(c3)-log(c1))
				//	+c2*(5908.5*(c2-c3)*(c2-c3)+8131.0)*c3
				//	+26850.0*c2*c3
				//	+c2*(-1969.5*(c2-c3)*(c2-c3)*(c2-c3)-8131.0*(c2-c3)+9927.1)
				//	-33766.35316921635);
				//----- -----
				//dGg/dCmn=dGg/dc3 (modified original code)
				//d(LG12)*c1*c2=LG12p*c1*c2
				//d(LG13)*c1*c3=LG13p*-2.0*c1*c3
				//d(LG14)*c1*c4=-LG14p1*c1*c4 +LG14p2*-2.0*(c1-c4)*c1*c4
				//d(LG23)*c2*c3=LG23p1*c2*c3 +LG23p2*3.0*(c2-c3)*(c2-c3)*c2*c3
				//d(LG34)*c3*c4=LG34p*c3*c4
				//=G0G3-G0G1
				//+LG12p*c1*c2
				//+LG13p*-2.0*c1*c3
				//-LG14p1*c1*c4 +LG14p2*-2.0*(c1-c4)*c1*c4
				//-LG12*c2-LG13*c3-LG14*c4
				//-LG23p1*c2*c3 +LG23p2*-3.0*(c2-c3)*(c2-c3)*c2*c3 +LG23*c2
				//+LG13*c1
				//+LG34p*c3*c4 +LG34*c4
				//-LG123*c2*c3+LG123*c1*c2 -LG124*c2*c4
				//+Gmg(omitted)+-RT*log(c1)-RT+RT*log(c3)+RT
				dgmng=beta*
					(G0G3-G0G1 //-33766.35316921635 ?
					+LG12p*c1*c2 //a part of -21177.185*c1*c2 = (LG12p+LG123)*c1*c2
					+LG13p*-2.0*c1*c3
					-LG14p1*c1*c4 +LG14p2*-2.0*(c1-c4)*c1*c4
					-LG12*c2
					-LG13*c3
					-LG14*c4
					-LG23p1*c2*c3 +LG23p2*-3.0*(c2-c3)*(c2-c3)*c2*c3
					+LG23*c2
					+LG13*c1
					+LG34p*c3*c4
					+LG34*c4
					-LG123*c2*c3
					+LG123*c1*c2 //a part of -21177.185*c1*c2 = (LG12p+LG123)*c1*c2
					-LG124*c2*c4
					+RT*(log(c3)-log(c1))
					);
				//----- ----- ----- ----- ----- -----
				//Functional derivative with respect to Ni concentration
				//dGg/dCni=dGg/dc4 (original in text)
				//dgmng=1.4613878411949395e-4*
				//	(6842.810456*(log(c4)-log(c1))
				//	-c2*(5672.815*(c4+c3+2*c2-1.0)+42968.802)
				//	+(2903.220969*(-2*c4-c3-c2+1.0)-14838.29558)*c1*c4
				//	-(-725.805174*(-2*c4-c3-c2+1.0)*(-2*c4-c3-c2+1.0)+7419.14778999*(-2*c4-c3-c2+1.0)-9359.746)*c4
				//	-6276.0*c3*c4
				//	+50709.624*c2*c4
				//	+(-725.805174*(-2*c4-c3-c2+1.0)*(-2*c4-c3-c2+1.0)+7419.14778999*(-2*c4-c3-c2+1.0)-9359.746)*c1
				//	+259.0*c3*c1
				//	-42168.485*c2*c1
				//	+c3*(6276.0*(c3-c4)-49205.406)
				//	-c3*(-259.0*(-c4-2*c3-c2+1.0)-4581.105)
				//	+c2*(10672.046-2868.324*(c2-c4))
				//	+26850.0*c2*c3
				//	+566.3008361);
				//----- -----
				//dGg/dCni=dGg/dc4 (modified original code)
				//d(LG12)*c1*c2=LG12p*c1*c2
				//d(LG13)*c1*c3=-LG13p*c1*c3
				//d(LG14)*c1*c4=LG14p1*-2.0*c1*c4 +LG14p2*2.0*(c1-c4)*-2.0*c1*c4
				//d(LG24)*c2*c4=-LG24p*c2*c4
				//d(LG34)*c3*c4=-LG34p*c3*c4
				//=G0G4-G0G1
				//+LG12p*c1*c2
				//-LG13p*c1*c3
				//+LG14p1*-2.0*c1*c4 +LG14p2*2.0*(c1-c4)*-2.0*c1*c4
				//-LG24p*c2*c4
				//-LG34p*c3*c4
				//-LG12*c2 -LG13*c3 -LG14*c4
				//+LG14*c1 +LG24*c2 +LG34*c3
				//-LG123*c2*c3 -LG124*c2*c4+LG124*c1*c2 
				//+Gmg(omitted)+-RT*log(c1)-RT+RT*log(c4)+RT
				dgnig=beta*
					(G0G4-G0G1 //+566.3008361 ?
					+LG12p*c1*c2 //a part of -42168.485*c1*c2 = (LG12p+LG124)*c1*c2
					-LG13p*c1*c3
					+LG14p1*-2.0*c1*c4 +LG14p2*2.0*(c1-c4)*-2.0*c1*c4
					-LG24p*c2*c4 //a part of +50709.624*c2*c4 = -(LG24p+LG124)*c2*c4
					-LG34p*c3*c4
					-LG12*c2
					-LG13*c3
					-LG14*c4
					+LG14*c1
					+LG24*c2
					+LG34*c3
					-LG123*c2*c3
					-LG124*c2*c4 //a part of +50709.624*c2*c4 = -(LG24p+LG124)*c2*c4
					+LG124*c1*c2 //a part of -42168.485*c1*c2 = (LG12p+LG124)*c1*c2
					+RT*(log(c4)-log(c1))
					);
				//----- ----- ----- ----- ----- -----
				
				/* Calculate the derivatives of free energy which
				   will be used later for the evaluation of 
				   derivative with respect to order parameter */
				//
				// free energy for Alpha gamma
				//
				// G of alpha phase (original in text)
				//gcunia=1.4613878411949395e-4*
				//	(6842.810456*(c4*log(c4)+log(c1)*c1+c3*log(c3)+c2*log(c2))
				//	+(201.36424*(-2*c4-c3-c2+1.0)-2016.04498)*c1*c4
				//	+c3*(6276.0*(c3-c4)-48642.59)*c4
				//	+10672.046*c2*c4
				//	+5788.496*c4
				//	+30000.0*c2*c3*c1
				//	-1740.949*c3*c1
				//	+36076.894*c2*c1
				//	+c2*(6252.0-9865.0*(c2-c3))*c3
				//	-33919.8913*c3
				//	+2984.135*c2);
				//----- -----
				// G of alpha phase (modified original code)
				gcunia=beta*
					(G0A1*c1
					+G0A2*c2
					+G0A3*c3 //-33919.8913*c3 is a mistake ?
					+G0A4*c4 //+5788.496*c4 is a mistake ?
					+LA12*c1*c2
					+LA13*c1*c3
					+LA14*c1*c4
					+LA23*c2*c3
					+LA24*c2*c4 //10672.046*c2*c4 is a mistake ?
					+LA34*c3*c4
					+LA123*c1*c2*c3
					+RT*(c1*log(c1)+c2*log(c2)+c3*log(c3)+c4*log(c4))
					);
				
				// G of gamma phase (original in text)
				//gcunig=1.4613878411949395e-4*
				//	(6842.810456*(c4*log(c4)+log(c1)*c1+c3*log(c3)+c2*log(c2))
				//	+c2*c1*(5672.815*(c4+c3+2*c2-1.0)+42968.802)
				//	+(-725.805174*(-2*c4-c3-c2+1.0)*(-2*c4-c3-c2+1.0)+7419.147789*(-2*c4-c3-c2+1.0)-9359.746)*c1*c4
				//	-47841.3*c2*c1*c4
				//	+c3*(6276.0*(c3-c4)-49205.406)*c4
				//	+c2*(10672.046-2868.324*(c2-c4))*c4
				//	+c3*(-259.0*(-c4-2*c3-c2+1.0)-4581.105)*c1
				//	-26850.0*c2*c3*c1
				//	-566.300*c1
				//	+c2*(-1969.5*(c2-c3)*(c2-c3)*(c2-c3)-8131.0*(c2-c3)+9927.1)*c3
				//	-34332.654*c3);
				//----- -----
				// G of gamma phase (modified original code)
				gcunig=beta*
					(G0G1*c1 //-566.300*c1 is a mistake ?
					+G0G2*c2
					+G0G3*c3 //-34332.654*c3 is a mistake ?
					+G0G4*c4
					+LG12*c1*c2
					+LG13*c1*c3
					+LG14*c1*c4
					+LG23*c2*c3
					+LG24*c2*c4
					+LG34*c3*c4
					+LG123+c1*c2*c3
					+LG124*c1*c2*c4
					+RT*(c1*log(c1)+c2*log(c2)+c3*log(c3)+c4*log(c4))
					);
			}else{
				/* If Fe concentration is greater than 0.9995 or less than 0.0005 apply
				   a penalty term. This is a numerical trick to avoid the overflows
				   because of presence of log terms in free energy functional Eq.5.26 */
				
				if(c1>=0.9995){
					thrc=(c1-0.9995);
				}
				
				if(c1<=0.0005){
					thrc=(c1-0.0005);
				}
				
				//Calculate the derivatives of penalty term for each alloying elememnts
				gcuni=Coef*thrc*thrc;
				//----- -----
				gcunia=gcuni; //gcunia=Coef*thrc*thrc;
				gcunig=gcuni; //gcunig=Coef*thrc*thrc;
				//----- -----
				
				//Derivatives of free energy based on function h for each alloying elements
				dgcu=-2.0*Coef*thrc;
				//----- -----
				dgcua=dgcu; //dgcua=-2.0*Coef*thrc;
				dgcug=dgcu; //dgcug=-2.0*Coef*thrc;
				//
				dgmna=dgcu; //dgmna=-2.0*Coef*thrc;
				dgmng=dgcu; //dgmng=-2.0*Coef*thrc;
				//
				dgnia=dgcu; //dgnia=-2.0*Coef*thrc;
				dgnig=dgcu; //dgnig=-2.0*Coef*thrc;
				//----- -----
			}//end if
			
			//Derivative of free energy based on the function h for Cu,Mn,and,Ni concentration, respectively
			// d( [1-h(eta)]*[Galpha+Y*Vm*epsilon0(ci)^2] + h(eta)*Ggamma )/d(ci) and h(eta)=funch
			dgdcu[ii]=(1.0-funch)*(dgcua+delascu)+funch*dgcug;
			dgdmn[ii]=(1.0-funch)*(dgmna+delasmn)+funch*dgmng;
			dgdni[ii]=(1.0-funch)*(dgnia+delasni)+funch*dgnig;
			
			//Derivative of free energy with respsect to order parameter
			/* d( [1-h(eta)]*[Galpha+Y*Vm*epsilon0(ci)^2] + h(eta)*Ggamma + W*g(eta)^2)/d(eta)
			   h(eta)=eta*eta*(3-2*eta)=funch, g(eta)=eta*(1-eta)=funcg
			   d(h)/d(eta) = 2.0*eta*(3-2*eta) + eta*eta*-2.0 = 6.0*eta*(1.0-eta) = dhdor
			   d(g^2)/d(eta)=2.0*g*(1-2.0*eta) = 2.0*funcg*(1-2.0*eta) = funcg*(2.0-4.0*eta)
			   (gcunia+elaste) = [Galpha+Y*Vm*epsilon0(ci)^2]
			   gcunig = Ggamma, W = constw, eta=orp[ii]                                                  */
			//dgdor[ii][0]=(gcunia+elaste)*( 2.0*orp[ii]*(3.0-2.0*orp[ii]) + orp[ii]*orp[ii]*-2.0 )*-1.0
			//				  +gcunig*( 2.0*orp[ii]*(3.0-2.0*orp[ii]) + orp[ii]*orp[ii]*-2.0 )
			//				  +constw*( 2.0*orp[ii]*(1.0-orp[ii])*(1.0-2.0*orp[ii]) );
			dhdor=6.0*orp[ii]*(1.0-orp[ii]);
			dgdor[ii]=(gcunia+elaste)*( -dhdor ) // -(6.0*orp[ii]-6.0*orp[ii]*orp[ii])
							  +gcunig*(  dhdor ) //  (6.0*orp[ii]-6.0*orp[ii]*orp[ii])
							  +constw*( funcg*(2.0-4.0*orp[ii]) );
		}//j
	}//i
	return;
}