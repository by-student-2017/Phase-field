...
int main(){
	...
	cudaMalloc((void**)&cu_d,    sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&cuk_d,   sizeof(cufftComplex)*Nx*Ny);
	//
	cufftComplex *mn_d, *mnk_d; // Mn
	cudaMalloc((void**)&mn_d,    sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&mnk_d,   sizeof(cufftComplex)*Nx*Ny);
	//
	cufftComplex *ni_d, *nik_d; // Ni
	cudaMalloc((void**)&ni_d,    sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&nik_d,   sizeof(cufftComplex)*Nx*Ny);
	//
	cufftComplex *orp_d, *orpk_d; // order parameter
	cudaMalloc((void**)&orp_d,   sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&orpk_d,  sizeof(cufftComplex)*Nx*Ny);
	//----- ----- ----- ----- 
	cufftComplex *dgdcu_d, *dgdcuk_d; // dG/dCu
	cudaMalloc((void**)&dgdcu_d,  sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&dgdcuk_d, sizeof(cufftComplex)*Nx*Ny);
	//
	cufftComplex *dgdmn_d, *dgdmnk_d; // dG/dMn
	cudaMalloc((void**)&dgdmn_d,  sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&dgdmnk_d, sizeof(cufftComplex)*Nx*Ny);
	//
	cufftComplex *dgdni_d, *dgdnik_d; // dG/dNi
	cudaMalloc((void**)&dgdni_d,  sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&dgdnik_d, sizeof(cufftComplex)*Nx*Ny);
	//
	cufftComplex *dgdor_d, *dgdork_d; // dG/dor
	cudaMalloc((void**)&dgdor_d,  sizeof(cufftComplex)*Nx*Ny);
	cudaMalloc((void**)&dgdork_d, sizeof(cufftComplex)*Nx*Ny);
	//----- ----- ----- ----- 
	...
	//float  cu[Nx][Ny];
	 float *cu = (float *)malloc(sizeof(float)*( Nx*Ny ));
	//float  mn[Nx][Ny];
	 float *mn = (float *)malloc(sizeof(float)*( Nx*Ny ));
	//float  ni[Nx][Ny];
	 float *ni = (float *)malloc(sizeof(float)*( Nx*Ny ));
	//float orp[Nx][Ny];
	float *orp = (float *)malloc(sizeof(float)*( Nx*Ny ));

	//prepare microstructure
	init_FeCuMnNi_micro_2d(Nx,Ny,cu0,mn0,ni0,cu,mn,ni,orp);
	
	float _Complex  *cuc   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Cu
	float _Complex  *mnc   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Mn
	float _Complex  *nic   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Ni
	float _Complex *orpc   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Order parameter
	
	float _Complex  *cuk   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Cu
	float _Complex  *mnk   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Mn
	float _Complex  *nik   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Ni
	float _Complex *orpk   = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // Order parameter
	
	//float dgdcu[Nx][Ny];
	float *dgdcu = (float *)malloc(sizeof(float)*( Nx*Ny ));
	//float dgdmn[Nx][Ny];
	float *dgdmn = (float *)malloc(sizeof(float)*( Nx*Ny ));
	//float dgdni[Nx][Ny];
	float *dgdni = (float *)malloc(sizeof(float)*( Nx*Ny ));
	//float dgdor[Nx][Ny];
	float *dgdor = (float *)malloc(sizeof(float)*( Nx*Ny ));
	
	float _Complex *dgdcuc = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny ));
	float _Complex *dgdmnc = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny ));
	float _Complex *dgdnic = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny ));
	float _Complex *dgdorc = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny ));
	
	float _Complex *dgdcuk = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // dG/dCu
	float _Complex *dgdmnk = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // dG/dMn
	float _Complex *dgdnik = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // dG/dNi
	float _Complex *dgdork = (float _Complex *)malloc(sizeof(float _Complex)*( Nx*Ny )); // dG/d(Order parameter)
	...
	for(int istep=0;istep<=nstep;istep++){
		...
		//
		cudaMemcpy(cu_d,cuc,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //cuc = cuc_h, Cu
		cudaMemcpy(mn_d,mnc,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //mnc = mnc_h, Mn
		cudaMemcpy(ni_d,nic,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //nic = nic_h, Ni
		cudaMemcpy(orp_d,orpc,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //orpc = orpc_h, Order parameter
		//
		...
		//----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
		//
		cudaMemcpy(cuk,cuk_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //cu = cu_h, Cu
		cudaMemcpy(mnk,mnk_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //mn = mn_h, Mn
		cudaMemcpy(nik,nik_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //ni = ni_h, Ni
		cudaMemcpy(orpk,orpk_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //orp = orp_h, Order
		...
		//
		cudaMemcpy(dgdcu_d,dgdcuc,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //dgdcuc = dgdcuc_h, dG/dCu
		cudaMemcpy(dgdmn_d,dgdmnc,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //dgdmnc = dgdmnc_h, dG/dMn
		cudaMemcpy(dgdni_d,dgdnic,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //dgdnic = dgdnic_h, dG/dNi
		cudaMemcpy(dgdor_d,dgdorc,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //dgdorc = dgdorc_h, dG/d(Order parameter)
		//
		...
		//----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
		//
		cudaMemcpy(dgdcuk,dgdcuk_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //dgdcu = dgdcu_h, dG/dCu
		cudaMemcpy(dgdmnk,dgdmnk_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //dgdmn = dgdmn_h, dG/dMn
		cudaMemcpy(dgdnik,dgdnik_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //dgdni = dgdni_h, dG/dNi
		cudaMemcpy(dgdork,dgdork_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //dgdor = dgdor_h, dG/d(Order parameter)
		...
		for(int i=0;i<Nx;i++){
			for(int j=0;j<Ny;j++){
				...
				// These are related with Eq.5.14
				//----- ----- ----- -----
		...
		cudaMemcpy(cuk_d,cuk,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //cuk = cuk_h, Cu
		cudaMemcpy(mnk_d,mnk,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //mnk = mnk_h, Mn
		cudaMemcpy(nik_d,nik,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //nik = nik_h, Ni
		cudaMemcpy(orpk_d,orpk,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //orpk = orpk_h, Order
		...
		cudaMemcpy(cuc,cu_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //cuc = cuc_h, Cu
		cudaMemcpy(mnc,mn_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //mnc = mnc_h, Mn
		cudaMemcpy(nic,ni_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //nic = nic_h, Ni
		cudaMemcpy(orpc,orp_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //orpc = orpc_h, Order parameter
