	...
	for(int i=0;i<Nx;i++){
		for(int j=0;j<Ny;j++){
			for(int k=0;k<Nz;k++){
				ii=i*Ny*Nz+j*Nz+k;
				//
				 cu[ii] =   cu0 + noise*(0.5-(double)rand()/RAND_MAX);
				 mn[ii] =   mn0 + noise*(0.5-(double)rand()/RAND_MAX);
				 ni[ii] =   ni0 + noise*(0.5-(double)rand()/RAND_MAX);
				//
				orp[ii] = 0.001 + noise*(0.5-(double)rand()/RAND_MAX);
			}
		}
	}
