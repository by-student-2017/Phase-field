...
int main(){
	...
	//----- for cufft
	float *k2_d, *k4_d;
	k2_d  = (float *)malloc(Nx*Ny*Nz*sizeof(float));
	k4_d  = (float *)malloc(Nx*Ny*Nz*sizeof(float));
	cudaMalloc((void**)&k2_d ,Nx*Ny*Nz*sizeof(float));
	cudaMalloc((void**)&k4_d ,Nx*Ny*Nz*sizeof(float));
	cudaMemcpy(k2_d,k2,Nx*Ny*Nz*sizeof(float),cudaMemcpyHostToDevice); //k2 = k2_h
	cudaMemcpy(k4_d,k4,Nx*Ny*Nz*sizeof(float),cudaMemcpyHostToDevice); //k4 = k4_h
	//----- ----- ----- -----
	...
	for(int istep=0;istep<=nstep;istep++){
		...
		for(int i=0;i<Nx;i++){
			for(int j=0;j<Ny;j++){
				for(int k=0;k<Nz;k++){
					...
					delsdcc[ijk] = delsdc[ijk]/RT; //Normalize the derivative elastic energy with RT
					crc[ijk] = cr[ijk];
				}
			}
		}
		...
		//----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
		//dfdcrk=fft3(dfdcr);        //Octave or Matlab
		//----- ----- ----- -----
		...
		//----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
		//cr=real(ifft3(crk));     //Octave or Matlab
		//----- ----- ----- -----
