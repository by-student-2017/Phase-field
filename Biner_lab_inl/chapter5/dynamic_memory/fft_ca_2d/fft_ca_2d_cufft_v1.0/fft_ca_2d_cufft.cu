	...
	for(int istep=0;istep<=nstep;istep++){
		...
		for(int igrain=0;igrain<ngrain;igrain++){
			...
			if(glist[igrain]==1){
				...
				cudaMemcpy(etak,etak_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //etak = etak_h
				cudaMemcpy(dfdetak,dfdetak_d,Nx*Ny*sizeof(float _Complex),cudaMemcpyDeviceToHost); //dfdetak = dfdetak_h
				...
				for(int i=0;i<Nx;i++){
					for(int j=0;j<Ny;j++){
						...
						//
						numer=dtime*mobil*dfdetak[ij];
						etak[ij]=(etak[ij]-numer)/denom;
					}
				}
				cudaMemcpy(etak_d,etak,Nx*Ny*sizeof(float _Complex),cudaMemcpyHostToDevice); //etak = etak_h
				//----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
