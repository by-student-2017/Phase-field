...
int main(){
	...
	//
	float _Complex *dfdetak = (float _Complex *)malloc(sizeof(float _Complex)*( NxNyNz ));
	float _Complex *etak    = (float _Complex *)malloc(sizeof(float _Complex)*( NxNyNz ));
	...
	for(int istep=0;istep<=nstep;istep++){
		...
		for(int igrain=0;igrain<ngrain;igrain++){
			...
			if(glist[igrain]==1){
				...
				cudaMemcpy(etak,etak_d,NxNyNz*sizeof(float _Complex),cudaMemcpyDeviceToHost); //etak = etak_h
				cudaMemcpy(dfdetak,dfdetak_d,NxNyNz*sizeof(float _Complex),cudaMemcpyDeviceToHost); //dfdetak = dfdetak_h
				...
				for(int i=0;i<Nx;i++){
					for(int j=0;j<Ny;j++){
						for(int k=0;k<Nz;k++){
							...
							//
							numer=dtime*mobil*dfdetak[ijk];
							etak[ijk]=(etak[ijk]-numer)/denom;
						}
					}
				}
				cudaMemcpy(etak_d,etak,NxNyNz*sizeof(float _Complex),cudaMemcpyHostToDevice); //etak = etak_h
				//----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- 
