...
void micro_poly_cell_3d(int Nx, int Ny, int Nz, double R,
	...
	//ncell=ncell=80
	//double xc_1d[80];
	double *xc_1d = (double *)malloc(sizeof(double)*( ncell ));
	//double yc_1d[80];
	double *yc_1d = (double *)malloc(sizeof(double)*( ncell ));
	//double zc_1d[80];
	double *zc_1d = (double *)malloc(sizeof(double)*( ncell ));
	//----- ----- ----- -----
	int ncol=1;
	int nrow=2;
	...
	if(ncell>2){
		...
		for(iter=0;iter<500000;iter++){
			...
			znc=(double)Nz*( (double)rand()/RAND_MAX );
			...
			if(iflag==1){
				...
				zc_1d[(used_ncell-1)] = znc;
				
				for(int i=0;i<Nx;i++){
					for(int j=0;j<Ny;j++){
						for(int k=0;k<Nz;k++){
							if( ((i-xnc)*(i-xnc) + (j-ync)*(j-ync) + (k-znc)*(k-znc))<Rsq ){
								ijk=(i*Ny+j)*Nz+k;
								phis[ijk*ncell+(used_ncell-1)]=0.999;
							}
						}
					}
				}
	...
	if(ncell==2){
		...
		for(int icol=0;icol<ncol;icol++){
			for(int irow=0;irow<nrow;irow++){
				...
				for(int i=0;i<Nx;i++){
					for(int j=0;j<Ny;j++){
						if( ((i-dx)*(i-dx) + (j-dy)*(j-dy) + dz*dz)<R2 ){
							ijk=(i*Ny+j)*Nz+(int)Nz/2.0;
							phis[ijk*ncell+(icell-1)]=0.999;
						}
					}
				}
				//
