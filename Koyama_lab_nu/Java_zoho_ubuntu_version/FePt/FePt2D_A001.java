//*** [プログラム (FePt2D_A001.java)] ************************************************
//*** [インポート文] ****************************
import java.awt.*;
import java.awt.event.*;
import java.io.*;

public class FePt2D_A001 extends Frame{
//*** [グローバル変数] *****************************************************************************************
...
//*** [メインプログラム] *******************************************************************************************
	public static void main(String[] args) throws Exception{//例外処理は行わない
		...
		FePt2D_A001 prog=new FePt2D_A001();		//FePt2D_A001のインスタンスprogを生成
		...
		double AA0, AA1, AA2, AA3, AA4, AA5, AA6;		//化学的駆動力定数
...
//---- 外力の設定 ---------------------------------------------
		...
		//sig22_a=0.0;						//ここでは外力を0に設定
 		sig22_a=500.0*1.0e+06*vm0/RR/temp; //500MPa
...
//---- phase fieldの保存 ----------------------------------------------------
			...
			if((((int)(time1) % Nstep)==0)){ prog.datsave(); }	//カウント数がNstepの倍数おきに場を保存
			//if((((int)(time1) % 500)==0)){ prog.datsave(); }		//カウント数が200の倍数おきに場を保存
			...
			for(i=0;i<=ndm;i++){
				for(j=0;j<=ndm;j++){
					xr[i][j]=ep11h0[i][j]=eta_s1[1][1]*s1h[i][j]*s1h[i][j]
															 +eta_s2[1][1]*s2h[i][j]*s2h[i][j];		//式(5)
					xi[i][j]=0.0;
				}
			}
			...
			for(i=0;i<=ndm;i++){
				for(j=0;j<=ndm;j++){
					xr[i][j]=ep22h0[i][j]=eta_s1[2][2]*s1h[i][j]*s1h[i][j]
															 +eta_s2[2][2]*s2h[i][j]*s2h[i][j]; 	//式(5)
					xi[i][j]=0.0;
				}
			}
			qs=-1.0; prog.rcfft();		//実空間からフーリエ空間への変換(qs<0)
...
//****** 化学ポテンシャルの計算 ********************************
					s1k_chem=AA0*(AA1*s1+AA2*s1*s1*s1+AA3*s1*s1*s1*s1*s1
											 +AA4*s1*s2*s2+AA5*s1*(2.0*s1*s1*s2*s2+s2*s2*s2*s2) ); 		//式(2)
					s2k_chem=AA0*(AA1*s2+AA2*s2*s2*s2+AA3*s2*s2*s2*s2*s2
											 +AA4*s2*s1*s1+AA5*s2*(2.0*s2*s2*s1*s1+s1*s1*s1*s1) ); 		//式(2)
...
//****** phase fieldの時間発展の計算 ********************************
					//s1ddtt=-smob*(s1k_chem+s1k_su[i][j]); 				//式(12)
					//s2ddtt=-smob*(s2k_chem+s2k_su[i][j]); 				//式(12)
	...
	public void ini_field(){
		...
		for(i=0;i<=ndm;i++){
			for(j=0;j<=ndm;j++){
				...
				s2h[i][j]=fac1*(2.0*Math.random()-1.0);		//均一に核を置く場合
...
//**** [デ－タの読込み] ************************************
	private void datin() throws Exception{
		...
		//BufferedReader infile=new BufferedReader(new FileReader("test_selected_0.dat"));
		//BufferedReader infile=new BufferedReader(new FileReader("test_selected_100.dat"));
		BufferedReader infile=new BufferedReader(new FileReader("test_selected_10000.dat"));
																										//ファイルのオープン
