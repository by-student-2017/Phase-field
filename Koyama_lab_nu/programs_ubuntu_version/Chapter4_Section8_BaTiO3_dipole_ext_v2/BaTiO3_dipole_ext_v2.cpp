...
int main(void)
{
	...
	double E1_ex, E1_ex_0, E1_ex_0c; 		//外部電場
	double ep00;							//比誘電率
...
//--- 平衡分極モーメント値の計算 ----------------------
	...
	E1_ex_0=E1_ex_0c*vm0/RR/temp;//外部電場(x方向)

	//ep00=200.;//比誘電率

	//Add0=1.0/ep0/ep00*vm0/RR/temp;//双極子-双極子相互作用計算における係数[式(4.7.8)参照]
...
//*** sinおよびcosテ－ブル、ビット反転テーブル、および初期場の設定 ***************
	...
	if(readff == 0){
		ini000_s12(s1h, s2h, ND);	//時間0における分極モーメントの初期プロファイル
	} else {
...
start: ;
	...
	//if((int)(time1)==2000){datsave();} //特定計算カウントにおけるデ－タの保存
...
void datsave(double *s1h, double *s2h, int ND)
{
	...
	stream = fopen("test_ext.dat", "a");	//書き込み先のファイルを追記方式でオープン
	fprintf(stream, "%e \n", time1);	//計算カウント数の保存
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
