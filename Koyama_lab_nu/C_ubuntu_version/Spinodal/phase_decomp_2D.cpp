...
#define INXY 400				//描画window１辺のピクセルサイズ
	...
	void datsave_gnuplot();		//濃度デ－タ保存サブル－チン
	void datsave_paraview();	//濃度デ－タ保存サブル－チン

int main(void)
{
	int  i, j, k, l; 			//整数
	int ip, im, jp, jm, Nstep; 	//整数（i+1, i-1, j+1, j-1）
...
start: ;
	printf("time: %f \n", time1);
	//if((((int)(time1) % Nstep)==0)) {datsave_gnuplot();}	//200ル－プ毎に濃度場を保存
	if((((int)(time1) % Nstep)==0)) {datsave_paraview();}	//200ル－プ毎に濃度場を保存
	//if((((int)(time1) % Nstep)==0)) {graph_c();}	//200ル－プ毎に濃度場を描画
...
void datsave_gnuplot()
{
	...
	iout = iout + 1;
	sprintf(fName,"sp_result%06d.dat",iout);
	stream = fopen(fName, "w");

	fprintf(stream, "%e\n", time1);	//時間の書き込み
	for(j=0;j<=ndm;j++){
		for(i=0;i<=ndm;i++){
			fprintf(stream, "%5d  %5d  %e  ", i, j, ch[i][j]);	//濃度場の書き込み
		}
		fprintf(stream, "\n");
	}
	fclose(stream);
