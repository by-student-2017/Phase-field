...
//***** Potential field calculation ***********************************
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			...
			c3k_chem=om_13*(c1-c3)-om_12*c2+om_23*c2+(log(c3)-log(c1));//âªäwägéUÉ|ÉeÉìÉVÉÉÉã
...
void ini000(double *c2h, double *c3h, int ND)
{
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			//c2h[i][j]=c2a+0.01*(2.0*DRND(1)-1.0);//Set the concentration field with random numbers up to Å}1%
			//c3h[i][j]=c3a+0.01*(2.0*DRND(1)-1.0);
			c2h[i*ND+j]=c2a+0.01*(2.0*DRND(1)-1.0);//Set the concentration field with random numbers up to Å}1%
