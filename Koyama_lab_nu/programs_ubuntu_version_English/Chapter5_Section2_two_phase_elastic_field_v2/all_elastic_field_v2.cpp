...
//#define IG 7		//2^IG=ND
	...
	int qs;					//Distinction between Fourier transform (qs:-1) and inverse Fourier transform (qs:1)
...
void table(double *s, double *c, int *ik, int ND, int ig)
{
	...
	q=2.0*PI/nd;
	for(i=0;i<=nd2-1;i++){ c[i]=cos(q*i); s[i]=sin(q*i); }//Sin, Cos table

	ik[0]=0; mn=nd2; mc=1;
	for(i=1;i<=ig;i++){
		for(j=0;j<=mc-1;j++){ ik[j+mc]=ik[j]+mn; }	//bit reversal table
		mc*=2; mn/=2; 
	}
}
