...
void ini000_s12(double *s1h, double *s2h, int ND)
{
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			//s1h[i][j]=0.01*(2.0*DRND(1)-1.0);//Set the field with random numbers up to }1%
			//s2h[i][j]=0.01*(2.0*DRND(1)-1.0);
			s1h[i*ND+j]=0.01*(2.0*DRND(1)-1.0);//Set the field with random numbers up to }1%
