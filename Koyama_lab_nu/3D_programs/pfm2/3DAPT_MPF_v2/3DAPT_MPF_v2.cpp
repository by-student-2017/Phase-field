...
//****** reg data ****************************************
	...
	time1max= data[5];
	temp    = data[6];	// [K]
	vm0     = data[7];	// gamma=0.5*vm0/RR/temp/b1;
	delta   = data[8];	// Not delt !!! For, aaa, W1, M1
	amobi   = data[9];	// M1=amobi*PI*PI/(M0*delta);
	M0      = data[10];	// M1=amobi*PI*PI/(M0*delta);
	E0      = data[11];	// E1=E0/RR/temp;
	Nstep   = int(data[12]);
	printf("---------------------------------\n");
	//
	nd=ND;
	...
	double *ph   = (double *)malloc(sizeof(double)*( N*ND*ND*ND + ND*ND*ND + ND*ND + ND ));	//ph[1][i][j]～ph[n][i][j]:位置[i][j]におけるpf
	int    *qh   =       (int *)malloc(sizeof(int)*( N*ND*ND*ND + ND*ND*ND + ND*ND + ND ));	//qh[1][i][j]～qh[n][i][j]:位置[i][j]においておける粒番号
