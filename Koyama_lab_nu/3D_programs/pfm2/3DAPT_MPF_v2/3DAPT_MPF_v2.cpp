...
//****** reg data ****************************************
	...
	double *ph   = (double *)malloc(sizeof(double)*( N*ND*ND*ND + ND*ND*ND + ND*ND + ND ));	//ph[1][i][j][k]～ph[n][i][j][k]:位置[i][j][k]におけるpf
	int    *qh   =       (int *)malloc(sizeof(int)*( N*ND*ND*ND + ND*ND*ND + ND*ND + ND ));	//qh[1][i][j][k]～qh[n][i][j][k]:位置[i][j][k]においておける粒番号
