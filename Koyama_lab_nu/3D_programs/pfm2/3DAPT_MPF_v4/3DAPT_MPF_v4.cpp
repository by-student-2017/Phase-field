...
//****** reg data ****************************************
	...
	double *ph   = (double *)malloc(sizeof(double)*( GNP*ND*ND*ND ));	//ph[1][i][j][k]～ph[n][i][j][k]:位置[i][j][k]におけるpf
	int    *qh   =       (int *)malloc(sizeof(int)*( GNP*ND*ND*ND ));	//qh[1][i][j][k]～qh[n][i][j][k][k]:位置[i][j][k]においておける粒番号
	int    *n00  =       (int *)malloc(sizeof(int)*( ND*ND*ND ));	//位置[i][j][k]においてphが0ではない場合の個数
	int    *n00p =       (int *)malloc(sizeof(int)*( ND*ND*ND ));	//位置[i][j][k]におけるphおよびその周辺のphが0ではない場合の個数
		...
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				...
				for(kk=1;kk<=n00[ip*ND*ND+j*ND+k];kk++){
					kkk=qh[kk*ND*ND*ND+ip*ND*ND+j*ND+k]; //位置[ip][j][k]におけるk番目の粒番号
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				...
				//}
				for(kk=1;kk<=n00[ip*ND*ND+j*ND+k];kk++){
					kkk=qh[kk*ND*ND*ND+ip*ND*ND+j*ND+k]; //位置[ip][j][k]におけるkk番目の粒番号
					ii=1; for(l=1;l<=n00p[i*ND*ND+j*ND+k];l++){ if(qh[l*ND*ND*ND+i*ND*ND+j*ND+k]==kkk){ii=0;} }
         		    //方位kkkが、位置[i][j][k]の方位に在ればii=0、無ければii=1
					if(ii==1){
						n00p[i*ND*ND+j*ND+k]+=1; 
