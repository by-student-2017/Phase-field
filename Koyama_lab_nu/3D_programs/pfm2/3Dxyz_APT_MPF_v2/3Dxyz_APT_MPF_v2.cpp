...
//****** reg data ****************************************
	...
	double *ph   = (double *)malloc(sizeof(double)*( N*NDX*NDY*NDZ ));	//ph[1][i][j][k]～ph[n][i][j][k]:位置[i][j][k]におけるpf
	int    *qh   =       (int *)malloc(sizeof(int)*( N*NDX*NDY*NDZ ));	//qh[1][i][j][k]～qh[n][i][j][k]:位置[i][j][k]においておける粒番号
	int    *n00  =       (int *)malloc(sizeof(int)*( NDX*NDY*NDZ ));	//位置[i][j][k]においてphが0ではない場合の個数
	int    *n00p =       (int *)malloc(sizeof(int)*( NDX*NDY*NDZ ));	//位置[i][j][k]におけるphおよびその周辺のphが0ではない場合の個数
	// n00[i*ND*ND+j*ND+k], n00p[i*ND*ND+j*ND+k]
	//
	double *ph2  = (double *)malloc(sizeof(double)*( N*NDX*NDY*NDZ ));	// ph2[kk*ND*ND*ND+i*ND*ND+j*ND+k];
	int    *qh2  =       (int *)malloc(sizeof(int)*( N*NDX*NDY*NDZ ));	// qh2[kk*ND*ND*ND+i*ND*ND+j*ND+k];
	//
