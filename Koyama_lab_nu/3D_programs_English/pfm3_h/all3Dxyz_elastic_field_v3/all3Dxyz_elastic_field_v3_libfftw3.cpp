...
int main(void)
{
	...
	double c11, c22, c33, c12, c13, c23, c44, c55, c66; //Modulus
	//double ep000;				//Lattice mismatch
	double eta11, eta22, eta33, eta12, eta13, eta23;	//Lattice mismatch
...
//****** Setting of calculation conditions and material constants ****************************************
	...
	double *ch    = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//phase field
	//
	double *ep11c = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Strain
	double *ep22c = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ep33c = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ep12c = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ep13c = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ep23c = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	//
	double *sig11 = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Elastic stress
	double *sig22 = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *sig33 = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *sig12 = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *sig13 = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *sig23 = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	//
	double *Estr  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Elastic strain energy density
	//
	double *u1    = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Displacement
	double *u2    = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *u3    = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	//
	double *qrh1  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Fourier transform of the field
	double *qih1  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	//
	double *ec11  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Constrained strain array
	double *ec22  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ec33  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ec12  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ec13  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	double *ec23  = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	//
	double *xr    = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));	//Real or imaginary array of Fourier transform
	double *xi    = (double *)malloc(sizeof(double)*( NDX*NDY*NDZ ));
	//
...
//*** Lattice mismatch settings ***
	...
	eta_c[2][2]=eta22;
	eta_c[3][3]=eta33;
	eta_c[1][2]=eta_c[2][1]=eta12;
	eta_c[1][3]=eta_c[3][1]=eta13;
	eta_c[2][3]=eta_c[3][2]=eta23;
...
//***** Elastic constant of Fe (bcc) ****************************
	...
	//c12=c12*el_mag;
	//c44=c44*el_mag;
	c11=c11*el_mag; c22=c22*el_mag; c33=c33*el_mag;
	c12=c12*el_mag;
	c13=c13*el_mag; c23=c23*el_mag;
	c44=c44*el_mag; c55=c55*el_mag;
	c66=c66*el_mag;
	...
	for(i=0;i<=ndxm;i++){
		for(j=0;j<=ndym;j++){
			for(k=0;k<=ndzm;k++){
				...
				ep11c[i*NDY*NDZ+j*NDZ+k]=ec11[i*NDY*NDZ+j*NDZ+k]+eta11*c0;
				ep22c[i*NDY*NDZ+j*NDZ+k]=ec22[i*NDY*NDZ+j*NDZ+k]+eta22*c0;
				ep33c[i*NDY*NDZ+j*NDZ+k]=ec33[i*NDY*NDZ+j*NDZ+k]+eta33*c0;
				ep12c[i*NDY*NDZ+j*NDZ+k]=ec12[i*NDY*NDZ+j*NDZ+k]+eta12*c0;
				ep13c[i*NDY*NDZ+j*NDZ+k]=ec13[i*NDY*NDZ+j*NDZ+k]+eta13*c0;
				ep23c[i*NDY*NDZ+j*NDZ+k]=ec23[i*NDY*NDZ+j*NDZ+k]+eta23*c0;
				...
				//
				sig11[i*NDY*NDZ+j*NDZ+k]=c11*ec11[i*NDY*NDZ+j*NDZ+k]+c12*ec22[i*NDY*NDZ+j*NDZ+k]+c13*ec33[i*NDY*NDZ+j*NDZ+k]
											   -(c11+c12+c13)*eta11*(ch[i*NDY*NDZ+j*NDZ+k]-c0);
				//c12=c21
				sig22[i*NDY*NDZ+j*NDZ+k]=c12*ec11[i*NDY*NDZ+j*NDZ+k]+c22*ec22[i*NDY*NDZ+j*NDZ+k]+c23*ec33[i*NDY*NDZ+j*NDZ+k]
											   -(c12+c22+c23)*eta22*(ch[i*NDY*NDZ+j*NDZ+k]-c0);
				//c13=c31, c23=c32
				sig33[i*NDY*NDZ+j*NDZ+k]=c13*ec11[i*NDY*NDZ+j*NDZ+k]+c23*ec22[i*NDY*NDZ+j*NDZ+k]+c33*ec33[i*NDY*NDZ+j*NDZ+k]
											   -(c13+c23+c33)*eta33*(ch[i*NDY*NDZ+j*NDZ+k]-c0);
				sig12[i*NDY*NDZ+j*NDZ+k]=c44*2.0*ec12[i*NDY*NDZ+j*NDZ+k]
											   -c44*2.0*eta12*(ch[i*NDY*NDZ+j*NDZ+k]-c0);
				sig13[i*NDY*NDZ+j*NDZ+k]=c55*2.0*ec13[i*NDY*NDZ+j*NDZ+k]
											   -c55*2.0*eta13*(ch[i*NDY*NDZ+j*NDZ+k]-c0);
				sig23[i*NDY*NDZ+j*NDZ+k]=c66*2.0*ec23[i*NDY*NDZ+j*NDZ+k]
				...
				Estr3=0.5*c11*(ep11c[i*NDY*NDZ+j*NDZ+k]*ep11c[i*NDY*NDZ+j*NDZ+k])
					 ...
					 +0.5*c33*(ep33c[i*NDY*NDZ+j*NDZ+k]*ep33c[i*NDY*NDZ+j*NDZ+k]);
			 ...
			 double *u1, double *u2, double *u3, int NDX, int NDY, int NDZ)
{
	...
	for(k=0;k<=ndzm;k++){
		for(j=0;j<=ndym;j++){
			for(i=0;i<=ndxm;i++){
				fprintf(fp,"%10.6f %10.6f %10.6f\n", u1[i*NDY*NDZ+j*NDZ+k], u2[i*NDY*NDZ+j*NDZ+k], u3[i*NDY*NDZ+j*NDZ+k]);
			}
		}
	}
	fclose(fp);
