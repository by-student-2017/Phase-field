...
//#define GNP 21
//#define GNP 1001
//#define INX 400						//Pixel size of drawing window 1 side x
//#define INY 800						//Pixel size of drawing window 1 side y
//#define INZ 400						//Pixel size of drawing window 1 side z
	...
	//double ph[N][NDX][NDY][NDZ];	//ph[1][i][j][K] to ph[n][i][j][K]: pf at position [i][j][K]
	//int qh[N][NDX][NDY][NDZ];	 	//qh[1][i][j][K] to qh[n][i][j][K]: grain number at position [i][j][K]
	//int n00[NDX][NDY][NDZ];			//number of cases where ph is not 0 at position [i][j][K]
	//int n00p[NDX][NDY][NDZ];		//Number of cases where ph at position [i][j][K] and surrounding ph is not 0
...
//****** reg data ****************************************
	...
	temp    = data[8];	// [K]
	dtemp   = data[9];
	vm0     = data[10];	// gamma=gamma0*vm0/RR/temp/b1;
	gamma0  = data[11];	// gamma=gamma0*vm0/RR/temp/b1;
	delta   = data[12];	// Not delt !!! For, K1, W1, M1
	K0      = data[13];	// 
	W0      = data[14];	// 
	amobi   = data[15];	// M1=amobi*PI*PI/(M0*delta);
	M0      = data[16];	// M1=amobi*PI*PI/(M0*delta);
	E0      = data[17];	// E1=E0/RR/temp;
	Nstep   = int(data[18]);
	readff  = int(data[19]);
	printf("---------------------------------\n");
	//
	...
	if ( gamma0==0.0 || amobi==0.0 || E0==0.0 ){
		...
		while(fgets(readline,132,fp) != NULL){
			if(k % (GN+1) != 0){
				...
				if( float(k/(GN+1))==4 && E0<0.0 ){
					printf("! ---------- Sij [dimensionless]\n");
					for(int j1=1;j1<=GN;j1++){
						printf("%8d ",j1);
					}
					printf("\n");
				...
				if( float(k/(GN+1))==8 && E0<0.0 ){
					printf("! ---------- cij, concentraion of No.ph \n");
					for(int j1=1;j1<=GN;j1++){
						printf("%8d ",j1);
					}
					printf("\n");
					...
					printf("! ---------- kij=ci/cj=cii/cjj, partion coefficient \n");
					for(int j1=1;j1<=GN;j1++){
						printf("%8d ",j1);
					}
					printf("\n");
					for(int i1=1;i1<=GN;i1++){
						...
						for(int j1=1;j1<=GN;j1++){printf("%9.4f ",kij[i1*GNP+j1]);}
						printf("\n");
					}
				}
				printf("%s",readline);
...
//****************************************************
	if(readff==0){
		...
		datin(ph, qh, n00, n00p, N, NDX, NDY, NDZ, GNP);
	}
	//datin();
 	//gwinsize(INX,INY); ginit(2); gsetorg(0,0);//Drawing window display
...
start: ;
	...
	if((((int)(time1) % Nstep)==0)) {datsave2(ph, qh, n00, n00p, N, NDX, NDY, NDZ, GNP);}
					...
					for(n1=1; n1<=n0p; n1++){
						...
						for(n2=1; n2<=n0p; n2++){
							...
							for(n3=1; n3<=n0p; n3++){
								...
								ch+=ph[n3*NDX*NDY*NDZ+i*NDY*NDZ+j*NDZ+k]*cij[kk*GNP+kk];
							}
		...
		for(j=0;j<=ndym;j++){
			for(k=0;k<=ndzm;k++){
				...
				for(kk=1;kk<=n00[ip*NDY*NDZ+j*NDZ+k];kk++){
					kkk=qh[kk*NDX*NDY*NDZ+ip*NDY*NDZ+j*NDZ+k]; //kth grain number at position [ip][j][k]
	...
	for(i=0;i<=ndxm;i++){
		for(j=0;j<=ndym;j++){
			for(k=0;k<=ndzm;k++){
				...
				//}
				for(kk=1;kk<=n00[ip*NDY*NDZ+j*NDZ+k];kk++){
					kkk=qh[kk*NDX*NDY*NDZ+ip*NDY*NDZ+j*NDZ+k]; //kkth grain number at position [ip][j][k]
					ii=1; for(l=1;l<=n00p[i*NDY*NDZ+j*NDZ+k];l++){ if(qh[l*NDX*NDY*NDZ+i*NDY*NDZ+j*NDZ+k]==kkk){ii=0;} }
         		    //ii=0 if the direction kkk is in the direction of the position [i][j][k], otherwise ii=1
					if(ii==1){
						   n00p[i*NDY*NDZ+j*NDZ+k]+=1; 
