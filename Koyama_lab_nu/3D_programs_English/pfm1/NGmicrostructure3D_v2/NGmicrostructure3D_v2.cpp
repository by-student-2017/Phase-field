...
//********************************************************************************
	...
	double *ch  = (double *)malloc(sizeof(double)*( ND*ND*ND + ND*ND + ND ));	//Concentration data array in tissu
	...
	//printf("Mx(1.0) = ");	scanf(" %lf",&Mx); 	//Mobility	Mx=1.0; 	//Standard value
	//printf("My(1.0) = ");	scanf(" %lf",&My); 	//Mobility	My=1.0; 	//Standard value
...
// 4: 1 torus (volume fraction is calculated here)
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				//ch[i][j][k]=c0+0.8*(2.*DRND(1)-1.);
				ch[i*ND*ND+j*ND+k]=c0+0.8*(2.*DRND(1)-1.);
				//ch[i][j][k]=c0+0.01*(2.*DRND(1)-1.);
				//if(ch[i][j][k]>=1.0){ch[i][j][k]=1.0-1.0e-06;}
				//if(ch[i][j][k]<=0.0){ch[i][j][k]=1.0e-06;}
				if(ch[i*ND*ND+j*ND+k]>=1.0){ch[i*ND*ND+j*ND+k]=1.0-1.0e-06;}
				if(ch[i*ND*ND+j*ND+k]<=0.0){ch[i*ND*ND+j*ND+k]=1.0e-06;}
	...
	//r0=nd/40;  //put a nucleus of diameter r0
	//PN=c0/( 4./3.*PI*(1./40.)*(1./40.)*(1./40.) );
