...
//****** Setting calculation conditions and material constants ****************************************
	...
	double *m00h = (double *)malloc(sizeof(double)*( N*ND*ND*ND + ND*ND*ND + ND*ND + ND ));	//number of directions where p is not 0 at location (i,j) and its surroundings (iÅ}1,jÅ}1)
	int    *n00h =       (int *)malloc(sizeof(int)*( ND*ND*ND + ND*ND + ND ));	//Number of orientations where p is not 0 at location (i,j) and its surroundings (iÅ}1,jÅ}1)
	//
