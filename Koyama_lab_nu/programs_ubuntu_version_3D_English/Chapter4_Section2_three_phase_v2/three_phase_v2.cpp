...
void ini000(double *c2h, double *c3h, int ND)
{
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				//c2h[i][j][k]=c2a+0.01*(2.0*DRND(1)-1.0);//Set the concentration field with random numbers up to Å}1%
				//c3h[i][j][k]=c3a+0.01*(2.0*DRND(1)-1.0);
				c2h[i*ND*ND+j*ND+k]=c2a+0.01*(2.0*DRND(1)-1.0);//Set the concentration field with random numbers up to Å}1%
