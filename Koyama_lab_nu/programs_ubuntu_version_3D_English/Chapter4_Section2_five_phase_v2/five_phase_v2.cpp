...
#define DRND(x) ((double)(x)/RAND_MAX*rand())  //Random number setting
	...
	double c2a, c3a, c4a, c5a;		//Average composition (1: A component, 2: B component, 3: C component)
	int Nstep, iout;
...
int main(void)
{
	...
	double c1, c2, c3, c4, c5;					//local concentration
	...
	double dc5a, sumc5;
	double sumct;
	double dakd2, dakd3, dakd4, dakd5;			//Second derivative of the diffusion potential
	double c2ddtt, c3ddtt, c4ddtt, c5ddtt;		//Time variation of concentration field
	...
	double cmob22, cmob33, cmob44, cmob55;		//mobility
	...
	double om_12, om_13, om_14, om_15;			//interaction parameter
	double om_23, om_24, om_25;
	double om_34, om_35;
	double om_45;
	double om_12e, om_13e, om_14e, om_15e;		//interaction parameter
	double om_23e, om_24e, om_25e;
	double om_34e, om_35e;
	double om_45e;
	double kapa_c1, kapa_c2, kapa_c3, kapa_c4, kapa_c5;		//concentration gradient energy coefficient
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				...
				c2=c2h[i*ND*ND+j*ND+k]; c3=c3h[i*ND*ND+j*ND+k]; c4=c4h[i*ND*ND+j*ND+k];  c5=c5h[i*ND*ND+j*ND+k];
				c1=1.0-c2-c3-c4-c5;//local concentration field

				//Gsys = Gc + Ggrad
				//Gc = integral { (L_AB * c1 * c2) + (L_AC * c1 * c3) + (L_BC * c2 * c3) + ...
				//  + RT*( c1*log(c1) + c2*log(c2) + c3*log(c3) + ...) }  dr
				c2k_chem=om_12*c1-om_12*c2-om_13*c3-om_14*c4-om_15*c5
						+om_23*c3+om_24*c4+om_25*c5+(log(c2)-log(c1));//chemical diffusion potential
				...
				c3k_chem=om_13*c1-om_12*c2-om_13*c3-om_14*c4-om_15*c5
						+om_23*c2+om_34*c4+om_35*c5+(log(c3)-log(c1));///chemical diffusion potential
				c3k_su=-2.0*(0.5*kapa_c1+0.5*kapa_c3)*div2_c3h
						   -(0.5*kapa_c1+0.5*kapa_c2)*div2_c2h
						   -(0.5*kapa_c1+0.5*kapa_c4)*div2_c4h
						   -(0.5*kapa_c1+0.5*kapa_c5)*div2_c5h;
				
				c4k_chem=om_14*c1-om_12*c2-om_13*c3-om_14*c4-om_15*c5
						+om_24*c2+om_34*c3+om_45*c5+(log(c4)-log(c1));///chemical diffusion potential
				c4k_su=-2.0*(0.5*kapa_c1+0.5*kapa_c4)*div2_c4h
						   -(0.5*kapa_c1+0.5*kapa_c2)*div2_c2h
						   -(0.5*kapa_c1+0.5*kapa_c3)*div2_c3h
						   -(0.5*kapa_c1+0.5*kapa_c5)*div2_c5h;
				
				c5k_chem=om_15*c1-om_12*c2-om_13*c3-om_14*c4-om_15*c5
						+om_25*c2+om_35*c3+om_45*c4+(log(c5)-log(c1));///chemical diffusion potential
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				...
				c2ddtt=cmob22*dakd2+cmob23*dakd3+cmob24*dakd4+cmob25*dakd5;//Diffusion equation (equation (4.2))
				c3ddtt=cmob32*dakd2+cmob33*dakd3+cmob34*dakd4+cmob35*dakd5;
				c4ddtt=cmob42*dakd2+cmob43*dakd3+cmob44*dakd4+cmob45*dakd5;
				c5ddtt=cmob52*dakd2+cmob53*dakd3+cmob54*dakd4+cmob55*dakd5;
...
//*** Concentration field balance correction ***********************************************
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				sumct=c2h[i*ND*ND+j*ND+k]+c3h[i*ND*ND+j*ND+k]+c4h[i*ND*ND+j*ND+k]+c5h[i*ND*ND+j*ND+k];
				if(sumct>=1.0){
...
void ini000(double *c2h, double *c3h, double *c4h, double *c5h, int ND)
{
	int i, j, k, id;
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				...
				fprintf(stream, "%e  %e  %e  %e  ", c2h[i*ND*ND+j*ND+k], 
...
void datsave_paraview(double *c2h, double *c3h, double *c4h, double *c5h, int ND)
{
	FILE	*fp;
	char	fName[256];
	int 	i, j, k;
...
void datin(double *c2h, double *c3h, double *c4h, double *c5h, int ND)
{
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				fscanf(datin0, "%le  %le  %le  %le  ", &c2h[i*ND*ND+j*ND+k], 
					&c3h[i*ND*ND+j*ND+k], &c4h[i*ND*ND+j*ND+k], &c5h[i*ND*ND+j*ND+k]);//Conservation of local concentration fields
			}
		}
	}
