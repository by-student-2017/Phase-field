...
#define DRND(x) ((double)(x)/RAND_MAX*rand())  //Random number setting
	...
	void datin(double *c2h, double *c3h, double *c4h, double *c5h, double *c6h, int ND);	//Subroutine for initial field reading
...
//****** Setting calculation conditions and material constants ****************************************
	...
	double *c6h  = (double *)malloc(sizeof(double)*( ND*ND*ND ));	//topical composition
	...
	double *c6h2 = (double *)malloc(sizeof(double)*( ND*ND*ND ));	//auxiliary matrix for the local concentration field
	...
	double *c6k  = (double *)malloc(sizeof(double)*( ND*ND*ND ));	//local potential
	//
	rtemp=rr*temp;				//RT
...
//************ Reading field data *******************************
void datin(double *c2h, double *c3h, double *c4h, double *c5h, double *c6h, int ND)
{
	...
	for(i=0;i<=ndm;i++){
		for(j=0;j<=ndm;j++){
			for(k=0;k<=ndm;k++){
				fscanf(datin0, "%le  %le  %le  %le  %le  ", &c2h[i*ND*ND+j*ND+k], 
					&c3h[i*ND*ND+j*ND+k], &c4h[i*ND*ND+j*ND+k], 
					&c5h[i*ND*ND+j*ND+k], &c6h[i*ND*ND+j*ND+k]);//Conservation of local concentration fields
			}
		}
