...
- Note 2: It is not designed to display all three components simultaneously. Change "concentration_A" to "concentration_B" or "concentration_C" in Paraview to display the distribution of each component. If you want to display the changes in each component in a single image, simply rewrite the code so that a value is added to each component in advance.
