...
- Recently, a method has been developed in which machine learning (particle methods such as Bayesian optimization) is used to find appropriate parameters while comparing images obtained from experiments with images from the phase-field method. For various reasons, it is difficult to achieve a high degree of fit with the phase-field method, as with the Rietveld method in XRD, so it is necessary to approach this method with the understanding that it is reproducing the experiment to some extent.
