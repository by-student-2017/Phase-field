...
## Features
- This github provides examples for not only "libfftw3" and "libfftw3-mpi" but also "cuFFT". By writing it in C, we were able to expand it to use "cuFFT". It is characterized by not only two -dimensional but also a 3D version of code.
...
- The output file "energy.out" has the first column as "Strain" and the second column as "energy density" (stretching in the x direction and shrinking in the y direction).
